...
class Icon(bs.Actor):
    def __init__(self,player,position,scale,showLives=True,showDeath=True,nameScale=1.0,nameMaxWidth=115.0,flatness=1.0,shadow=1.0):
...
class SafeZoneGame(bs.TeamGameActivity):
    ...
    def getSettings(cls,sessionType):
        return [("Time Limit",{'choices':[('None',0),('1 Minute',60),
                ...
                ("Lives Per Player",{'default':2,'minValue':1,'maxValue':10,'increment':1}),
                ("Respawn Times",{'choices':[('Short',0.5),('Normal',1.0)],'default':1.0}),
    ...
    def spawnZone(self):
        self.zonePos = (random.randrange(-10,10),0.0,random.randrange(-5,5))
        self.zone = bs.newNode('locator',attrs={'shape':'circle','position':self.zonePos,'color':(1, 1, 0),'opacity':0.5,'drawBeauty':True,'additive':False})
        self.zoneLimit = bs.newNode('locator',attrs={'shape':'circleOutline','position':self.zonePos,'color':(1, 0.2, 0.2),'opacity':0.8,'drawBeauty':True,'additive':False})
        bsUtils.animateArray(self.zone, 'size', 1,{0:[0], 300:[self.getPlayersCount()*0.85], 350:[self.getPlayersCount()*0.8]})
        bsUtils.animateArray(self.zoneLimit, 'size', 1,{0:[0], 300:[self.getPlayersCount()*1.2], 350:[self.getPlayersCount()*0.95]})
    ...
    def moveZone(self):
        if self.zonePos[0] > 0: x = random.randrange(0,10)
        else: x = random.randrange(-10,0)
		
        if self.zonePos[2] > 0: y = random.randrange(0,5)
        else: y = random.randrange(-5,0)
		
        newPos = (x,0.0,y)
        bsUtils.animateArray(self.zone, 'position', 3,{0:self.zone.position, 8000:newPos})
        bsUtils.animateArray(self.zoneLimit, 'position', 3,{0:self.zoneLimit.position,8000:newPos})
    ...
    def tick(self):
        ...
        self._time = bs.NodeActor(bs.newNode('text',
            ...
            'text':'Kill timer: '+str(self._timeRemaining)+'s',
            'opacity':0.8,'maxWidth':100,'hAlign':'center',
            'vAlign':'center','shadow':1.0,'flatness':1.0,
    ...
    def addBot(self,pos=None):
        ...
        self._bots.spawnBot(self.getRandomBot(),pos=position,spawnTime=1000,onSpawnCall=bs.Call(self._onBotSpawn))
